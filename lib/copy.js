...
function copy(srcPath, destPath, hasFinished, emitEvent, options) {
	...
	return prepareForCopy(srcPath, destPath, options)
		.then(function(result) {
			...
			var copyFunction = getCopyFunction(result.stats, hasFinished, emitEvent);
			return copyFunction(srcPath, destPath, result.stats, options, result.isWriteable);
		})
		.catch(function(error) {
			if (error instanceof CopyError) {
				throw error;
...
function prepareForCopy(srcPath, destPath, options) {
	...
	var skipIfExists = Boolean(options.skipIfExists);
	return (shouldExpandSymlinks ? stat : lstat)(srcPath)
		.then(function(stats) {
			return ensureDestinationIsWritable(destPath, stats, shouldOverwriteExistingFiles, skipIfExists)
				.then(function(isWriteable) {
					var returnObj = {
...
function ensureDestinationIsWritable(destPath, srcStats, shouldOverwriteExistingFiles, skipIfExists) {
...
function createCopyFunction(fn, stats, hasFinished, emitEvent, events) {
	...
	return function(srcPath, destPath, stats, options, isWriteable) {
		...
		return ensureDirectoryExists(parentDirectory)
			.then(function() {
				return fn(srcPath, destPath, stats, options, isWriteable);
			})
			.then(function() {
...
function copyFile(srcPath, destPath, stats, options, isWriteable) {
	return new Promise(function(resolve, reject) {
		if (isWriteable) {
			var hasFinished = false;
